        ...
        <div class="hist-section" v-if="!cargando">
            ...
            <div class="hist-filters">
                ...
                <div class="hist-filter-group">
                    ...
                    <input id="h_desde" type="date" v-model="histDesde" />
                </div>
                <div class="hist-filter-group">
                    <label for="h_hasta">Hasta</label>
                    <input id="h_hasta" type="date" v-model="histHasta" />
                </div>
...
onMounted(async () => {
    try {
        ...
        console.log("Asignaciones recibidas del backend:", res.data);
...
const tomarAsistencia = async () => {
    ...
    try {
        ...
        if (res.success && res.data && res.data.length > 0) {
            ...
            listaAlumnos.value = [];
            alert("No se encontraron alumnos registrados en este curso.");
        }
    } catch (error) {
        console.error("Error al obtener alumnos del curso:", error);
        alert("Ocurrió un error al intentar cargar la lista de alumnos.");
    }
};

const guardarAsistencia = async () => {
    if (!asistFecha.value) return alert("Selecciona una fecha");
    if (!asistCursoId.value) return alert("Selecciona un curso");
    if (listaAlumnos.value.length === 0) return alert("No hay alumnos en la lista para guardar.");
    ...
    try {
        ...
        alert("Ocurrió un error de conexión al guardar las asistencias.");
    }
};
