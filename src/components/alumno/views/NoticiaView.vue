            ...
            <div class="news-left-section">
                <div class="news-header-box">
                    ...
                    <div class="news-header-text">
                        <h1>Noticias</h1>
                        <p>
                            Mantente informado sobre lo que sucede en la Técnica
                            2
                        </p>
                ...
                <div class="filter-bar">
                    <div class="filter-tabs">
                        <button v-for="cat in categoriasFiltro" :key="cat" class="filter-btn"
                            :class="{ active: filtroCategoria === cat }" @click="cambiarFiltro(cat)">
                            {{ cat }}
                        </button>
                    </div>
                        ...
                        <div class="news-card-content">
                            ...
                            <div class="news-card-meta">
                                <span><i class="far fa-calendar"></i>
                                    {{ formatearFecha(noticia.fecha) }}</span>
                                <span>•</span>
                                <span><i class="fas fa-user"></i> Publicado por:
                                    {{ noticia.autor }}</span>
                            </div>
                            <div class="card-actions" v-if="puedeCrear">
                                <button class="loan-btn" @click="editarNoticia(noticia)">
                                    <i class="fas fa-pencil-alt"></i> Editar
                                </button>
                                <button class="loan-btn btn-delete" @click="eliminarNoticia(noticia)">
                ...
                <div class="pagination-container" v-if="totalPaginas > 1">
                    ...
                    <ul class="pagination-list">
                        ...
                        <li v-for="p in totalPaginas" :key="p" class="pagination-item"
                    ...
                    <div class="widget-list">
                        <div v-for="(anuncio, i) in anunciosWidget" :key="i" class="widget-item">
                            ...
                            <div class="widget-item-info">
                                ...
                                <p>
                                    {{
                                        anuncio.descripcion.substring(0, 40)
                                    }}...
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="widget-list">
                        <div v-for="(evento, i) in eventosWidget" :key="i" class="event-calendar-item">
                            ...
                            <div class="widget-item-info">
                                <h4>{{ evento.titulo }}</h4>
                                <p>
                                    {{ evento.descripcion.substring(0, 40) }}...
                                </p>
                            </div>
                        </div>
                    </div>
...
const crearNoticia = () => {
    if (
        !nuevaNoticia.value.titulo.trim() ||
        !nuevaNoticia.value.descripcion.trim()
    )
    ...
    noticias.value.unshift({
        ...
        autor: authStore.usuario?.nombre || "Admin",
        fecha: new Date(),
    });
...
const eliminarNoticia = (noticia) =>
    (noticias.value = noticias.value.filter((n) => n !== noticia));
    ...
    let resultado =
        filtroCategoria.value !== "Todas"
            ? noticias.value.filter(
                (n) => n.categoria === filtroCategoria.value,
            )
            : [...noticias.value];
...
const noticiasPaginadas = computed(() =>
    noticiasFiltradasYOrdenadas.value.slice(
        (paginaActual.value - 1) * noticiasPorPagina,
        paginaActual.value * noticiasPorPagina,
    ),
);
const totalPaginas = computed(() =>
    Math.ceil(noticiasFiltradasYOrdenadas.value.length / noticiasPorPagina),
);
...
const anunciosWidget = computed(() =>
    noticias.value.filter((n) => n.categoria === "Anuncios"),
);
const eventosWidget = computed(() =>
    noticias.value.filter((n) => n.categoria === "Eventos"),
);
